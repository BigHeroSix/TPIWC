...
class selfComplementary extends  HTMLElement {
    ...
    connectedCallback() {
        ...
        this.search.onchange = _ => {
            let marcaSelected = this.response.find(e => {return e.nombre === this.search.value});
            console.log("se selecciono "+ this.response);
            this.evn = new CustomEvent('complete', {'composed': true, 'bubbles': true, 'detail': {'marca': marcaSelected}});
